...
std::string extractKeyFromJson(const std::string& json) {
    ...
    if (pos == std::string::npos) return "default";
    pos = json.find(":", pos);
    if (pos == std::string::npos) return "default";
    size_t startQuote = json.find("\"", pos);
    if (startQuote == std::string::npos) return "default";
    size_t endQuote = json.find("\"", startQuote + 1);
    if (endQuote == std::string::npos) return "default";
    return json.substr(startQuote + 1, endQuote - startQuote - 1);
}

std::unordered_map<std::string, RateLimiter::RateConfig> parseLimitsJson(const std::string& filename) {
    std::unordered_map<std::string, RateLimiter::RateConfig> rates;
    std::ifstream file(filename);
    if (!file.is_open()) return rates;
    ...
    while ((pos = content.find("\"", pos)) != std::string::npos) {
        ...
        if (key == "capacity" || key == "refillRate" || key == "default") continue;
        if (key.find('/') == std::string::npos && key != "default") {
            // Not a path-like key, might be a property of an outer object we didn't parse properly, but in our format keys have '/'
            // Actually, "default" doesn't have '/', but we skip it above or handle it. Let's just look for the block.
        }
        ...
        if (capPos == std::string::npos) break;
...
void handleClient(SOCKET clientSocket, RateLimiter* rateLimiter) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (headersEnd == std::string::npos) break; // Need more data for headers
            ...
            if (clPos != std::string::npos && clPos < headersEnd) {
                ...
                if (clEnd != std::string::npos && clEnd < headersEnd) {
                    contentLength = std::atoi(clientBuffer.substr(clPos + 16, clEnd - clPos - 16).c_str());
                }
            }
            ...
            std::string request = clientBuffer.substr(0, headersEnd + 4 + contentLength);
            ...
            if (request.find("POST /check") == 0) {
                ...
                std::string jsonResponse = "{\"allowed\": " + allowedStr + ", \"waitForMs\": " + std::to_string(waitForMs) + "}";
                ...
                for (const auto& pair : statusMap) {
                    ...
                    jsonResponse += "\"" + pair.first + "\": {\"currentCapacity\": " + std::to_string(pair.second.currentCapacity) + 
...
int main() {
    ...
    std::unordered_map<std::string, RateLimiter::RateConfig> keyRates = parseLimitsJson("limits.json");
    if (keyRates.empty()) {
        std::cout << "Warning: limits.json not found or empty. Using default rate of 50." << std::endl;
    } else {
        std::cout << "Loaded " << keyRates.size() << " token bucket limits from limits.json." << std::endl;
    }

    double defaultRate = 50.0;
    if (keyRates.count("default")) {
        defaultRate = keyRates["default"].capacity; // Use capacity as default rate for simplicity
